...
const sumFromFunction = summer(tall);

if (sumFromFunction === 15) {
...
let differansResult = andreTall[0];

for (let i = 1; i < andreTall.length; i++) {
    differansResult -= andreTall[i];
}

console.log(differansResult);
...
const diffFromFunction = differansier(andreTall);

if (diffFromFunction === -18) {
...
const productResult = multipliser([2, 3, 4]);
